        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '工作人员名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.gongzuoName" placeholder="工作人员名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('gongzuo','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/hongshegemingwenwu/upload/gongzuoMuBan.xls"
                        >批量导入工作人员数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('gongzuo','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="gongzuoUploadSuccess"
                                :on-error="gongzuoUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('gongzuo','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "gongzuo.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="工作人员手机号">
                        <template slot-scope="scope">
                            {{scope.row.gongzuoPhone}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="gongzuoIdNumber"
                                   header-align="center"
                                   label="工作人员身份证号">
                        <template slot-scope="scope">
                            {{scope.row.gongzuoIdNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="gongzuoPhoto"
                                     header-align="center"
                                     width="200"
                                     label="工作人员头像">
                        <template slot-scope="scope">
                            <div v-if="scope.row.gongzuoPhoto">
                                <img :src="$base.url+scope.row.gongzuoPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('gongzuo','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('gongzuo','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('gongzuo','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('gongzuo','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                ...
                //本表字段
                     '账户': "username",
                     '工作人员名称': "gongzuoName",
                     '工作人员手机号': "gongzuoPhone",
                     '工作人员身份证号': "gongzuoIdNumber",
                     '工作人员头像': "gongzuoPhoto",
                     '性别': "sexValue",
                     '工作人员邮箱': "gongzuoEmail",
                     '添加时间': "insertTime",
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'gongzuo',//当前表表名,
                        sumColum : 'gongzuo_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'gongzuo_name',//分组字符串字段
                        // types : 'gongzuo_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.gongzuoName!= '' && this.searchForm.gongzuoName!= undefined) {
                    params['gongzuoName'] = '%' + this.searchForm.gongzuoName + '%'
                }
                ...
                params['gongzuoDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "gongzuo/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            gongzuoUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "gongzuo/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
